...
with sync_playwright() as pw:
    ...
    ok('홈 타일 이름 임금·주휴',p.inner_text('#wageOpen b').strip()=='임금·주휴',p.inner_text('#wageOpen b'))
    ...
    ok('지급 1,760,000·160시간·4회 → 2,112,000, 미지급 352,000, 주휴 미지급 문구',all(x in k for x in ['2,112,000원','352,000원','주휴수당 미지급 추정(근로기준법 제36조 또는 제43조)']),k)
    ...
    k=K(p); ok('공휴일 1일 → 88,000 더해 2,200,000, 미지급 440,000·공휴일 문구',all(x in k for x in ['공휴일 1일 × 8시간 × 11,000원','2,200,000원','440,000원','주휴수당·공휴일 유급휴일 수당 미지급 추정(근로기준법 제36조 또는 제43조)']),k)
