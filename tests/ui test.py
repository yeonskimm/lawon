...
def s8(p):   # 법령집: 고평법 시행규칙 자리
    ...
    ok('고평법 시행규칙은 고평법 바로 아래 하위 줄', 'EQ' in first and first.index('EQ') + 1 < len(first) and first[first.index('EQ') + 1] == 'EQR(하위)', first)
    tab(p, 'book'); p.click('#fieldsw'); p.wait_for_timeout(250)
    ...
    ok('산업안전일 때 그 밖의 법령에서도 고평법 바로 아래', 'EQR(하위)' in labs and labs.index('EQR(하위)') == labs.index('EQ') + 1, labs)
...
def s10(p):   # 법령 검색: 시행령·시행규칙 이름(2026-09-24 보완)
    ...
    ok("'고평법 시행규칙' 검색 → 시행규칙 조문 6개", n == 6, n)
...
def s11(p):   # 법령집: 고평법 시행규칙 별표1 — 2026-09-25부터 법령 줄의 별표 수 + 법령 안 [조문|별표] 탭
    p.click('.pick[data-field="labor"]'); tab(p, 'book')
    t = p.inner_text('#book [data-bk="EQR"] .bm')
    ok('법령집 고평법 시행규칙 줄: 별표 1(중복 없음)', '별표 1' in t and '1·1' not in t, t)
